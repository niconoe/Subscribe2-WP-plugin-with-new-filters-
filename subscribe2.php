	...
	*/
	function publish($post = 0, $preview = '') {
		...
		if ( $preview != '' ) {
			...
			$recipients = apply_filters('s2_send_plain_summary_suscribers', $recipients, $post->ID);
			$this->mail($recipients, $subject, $excerpt_body);

			// next we send plaintext full content emails
			$rec = $this->get_registered("cats=$post_cats_string&format=post&author=$post->post_author");
			$rec = apply_filters('s2_send_plain_fullcontent_suscribers', $rec, $post->ID);
			$this->mail($rec, $subject, $full_body);

			// next we send html excerpt content emails
			$rec = $this->get_registered("cats=$post_cats_string&format=html_excerpt&author=$post->post_author");
			$rec = apply_filters('s2_send_html_excerpt_suscribers', $rec, $post->ID);
			$this->mail($rec, $subject, $html_excerpt_body, 'html');

			// finally we send html full content emails
			$rec = $this->get_registered("cats=$post_cats_string&format=html&author=$post->post_author");
			$rec = apply_filters('s2_send_html_fullcontent_suscribers', $rec, $post->ID);
			$this->mail($rec, $subject, $html_body, 'html');
		}
	} // end publish()
